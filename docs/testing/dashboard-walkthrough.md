...
## History and operator-control follow-up (2026-09-25)

The production card in `tests/frontend/history-controls.html` passed the executable **Run history and control scenarios** check at desktop width and at 390 by 844. It verifies bounded history pages, search focus across typing and live updates, distinct terminal outcomes, escaped stored names, absorption links, retained-history expiry, explicit control expiry, maintenance scope preview, preview invalidation after edits or publication, preserved drafts, duplicate-submission prevention, ambiguous failures without automatic retries, disconnection, resolved targets and non-admin access. A closed history dialog stays closed after subsequent publication. The mobile maintenance form was visually inspected; fields, explanations and buttons fit without horizontal overflow.

`tests/test_dashboard_actions.py` exercises the real HA `call_service` WebSocket response wrapper for preview, maintenance and shelving; observes recovery in dashboard history; and rejects both mutations from a read-only user. Existing backend tests continue to cover persistence, expiry and attention semantics. The full check passed with 319 Python tests, 15 Node tests, strict types, lint, formatting and both coverage floors (98.79% combined). The browser fixture is a manual executable check, not part of Node-only CI.

This isolated increment starts from pilot commit `24b45dd`. Integration retry context, floor/area hierarchy and device-performance assessment are concurrent increments; combined-release validation belongs to the release task. No live-house controls or enrollment were changed during these checks.

## Remaining boundary

The dashboard remains administrator-only. The history/control follow-up adds bounded persisted history and forms using existing administrator services; early cancellation and acknowledgment remain unimplemented. These walkthroughs do not validate phone delivery, physical freshness, command completion, an external watchdog, packaged installation, or optional TopoMation integration. Large-inventory transfer/rendering remains unfinished. Combined-build validation and live installation belong to the release task.
